 ...
 * - Obstacles: Collide at circumference (player.radius + obstacle.radius)
...
class Obstacle {
    constructor(x, y, width, height, type = 'static') {
        ...
        this.height = height;
        this.type = type;
        this.collisionEffect = 'loseLife';
        this.color = '#ff4444';

        // Pre-calculate effective radius for collision detection (performance optimization)
        this.effectiveRadius = Math.sqrt(width * width + height * height) * 0.5;
    }
...
class Level {
    ...
    static _finalizeLevelGeneration(levelNumber, config, random, planets) {
        ...
        // Add obstacles
        const obstacleCount = config.obstacleCount[0] + Math.floor(random() * (config.obstacleCount[1] - config.obstacleCount[0] + 1));
        for (let i = 0; i < obstacleCount; i++) {
            ...
            while (!validPosition && attempts < 50) {
                width = 20 + Math.floor(random() * 60);
                height = 20 + Math.floor(random() * 60);
                x = width/2 + 10 + random() * (CANVAS_WIDTH - width - 20);
                y = height/2 + 10 + random() * (CANVAS_HEIGHT - height - 20);
                ...
                for (const planet of planets) {
                    ...
                    const dy = y - planet.y;
                    const dist = Math.sqrt(dx * dx + dy * dy);
                    if (dist < planet.radius + Math.max(width, height) / 2 + 40) {
                        validPosition = false;
                        break;
                    }
                }
                ...
                if (goalDist < 60) validPosition = false;

                // Check distance from start
                const startDist = Math.sqrt((x - startPosition.x) ** 2 + (y - startPosition.y) ** 2);
                if (startDist < 80) validPosition = false;

                attempts++;
            }

            if (validPosition) {
                level.obstacles.push(new Obstacle(x, y, width, height));
            }
        }
...
// Check collision with obstacles (uses swept collision for high speeds)
function checkCollision(player, obstacles) {
    ...
    for (let i = 0; i < obstacles.length; i++) {
        ...
        const combinedRadius = obstacle.effectiveRadius + pr;
        ...
        // Check swept collision (prevents tunneling at high speeds)
        if (lineSegmentIntersectsCircle(
...
function update(deltaTime) {
    ...
    if (checkCollision(gameState.player, gameState.currentLevel.obstacles)) {
        // Create death particle effect
        createParticleBurst(gameState.player.x, gameState.player.y, 15, 'rgba(255, 100, 0, 1)', 100);
        addScreenShake(10, 0.25);
